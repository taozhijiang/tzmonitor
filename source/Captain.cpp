...
#include <Protocol/ServiceImpl/MonitorTaskService.h>
...
bool Captain::init(const std::string& cfgFile) {
    ...
    net_server_ptr_.reset(new tzrpc::NetServer("tzrpc_server"));
    ...
    std::shared_ptr<tzrpc::Service> monitor_task_service = std::make_shared<tzrpc::MonitorTaskService>("MonitorTaskService");
    if (!monitor_task_service || !monitor_task_service->init()) {
        roo::log_err("create XtraTaskService failed.");
        return false;
    }
    tzrpc::Dispatcher::instance().register_service(tzrpc::ServiceID::MONITOR_TASK_SERVICE, monitor_task_service);

    // 再进行整体服务的初始化
    if (!tzrpc::Dispatcher::instance().init()) {
    ...
    if (!EventRepos::instance().init()) {
        roo::log_err("Init event repos failed.");
        return false;
    }
