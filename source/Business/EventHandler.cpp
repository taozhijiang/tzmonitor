...
bool EventHandler::init() {
    ...
    if (service_.empty()) {
        roo::log_err("service not initialized %s", service_.c_str());
        return false;
    }
    ...
    try {
        ...
        roo::log_info("EventHandlerConf for %s final info \n"
    ...
    if (!store_) {
        roo::log_err("store implement %s for %s not OK!",
    ...
    if (!thread_ptr_) {
        roo::log_err("create work thread failed! ");
        return false;
    }

    if (!Captain::instance().timer_ptr_->add_timer(std::bind(&EventHandler::linger_check_run, shared_from_this()), 500, true)) {
        roo::log_err("add linger_check_run failed.");
        return false;
    }
    ...
    try {
        ...
        for (int i = 0; i < rpc_handlers.getLength(); ++i) {
            ...
            if (instance_name == service_) {
                ...
                roo::log_warning("find specific conf for service %s", service_.c_str());
                ...
                roo::log_info("EventHandlerConf for service %s template info \n"
                              "event_linger %d, event_step %d, process_step_size %d, store_type %s",
                              service_.c_str(),
                              conf_.event_linger_,
                              conf_.event_step_,
...
int EventHandler::add_event(const event_report_t& ev) {
    ...
    if (ev.service != service_) {
        roo::log_err("error for service check, expect %s != %s",
    ...
    if (now - ev.timestamp > conf_.event_linger_) {
        roo::log_err("critical... too old report: %s %ld - %ld = %ld, drop it!",
...
int EventHandler::do_add_event(time_t ev_time, const std::vector<event_data_t>& data) {
    ...
    if (timed_iter == events_.end()) {
        roo::log_info("create new time slot: %ld", ev_time);
    ...
    for (auto iter = data.begin(); iter != data.end(); ++iter) {
        ...
        if (metric_iter == timed_slot.end()) {
            roo::log_info("create new metric %s at time_slot: %ld in %s",
...
int EventHandler::get_event(const event_cond_t& cond, event_select_t& stat) {
    ...
    if (!store_) {
        roo::log_err("store not initialized with storetype: %s", conf_.store_type_.c_str());
        return -1;
    }
...
void EventHandler::run() {
    ...
    roo::log_info("EventHandler thread %#lx begin to run ...", (long)pthread_self());
    ...
    while (true) {
        ...
        if (thread_terminate_) {
            roo::log_info("EventHandler thread (%s) %#lx about to terminate ...",
        ...
        while (process_queue_.SIZE() > 2 * queue_size) {
            ...
            if (!ret) {
                roo::log_err("Pop support task failed.");
                break;
            }
...
void calc_event_info_each_metric(std::vector<event_data_t>& data,
    ...
    if (ids.size() != data.size()) {
        roo::log_err("mismatch size, may contain duplicate items: %d - %d",
        ...
        for (size_t i = 0; i < data.size(); ++i) {
            for (size_t j = i + 1; j < data.size(); ++j) {
                if (data[i].msgid == data[j].msgid) {
                    roo::log_err("found dumpicate message: %lu - %lu, (%ld %ld %s), (%ld %ld %s)", i, j,
...
int EventHandler::do_process_event(events_by_time_ptr_t event, event_insert_t copy_stat) {
    ...
    for (auto iter = event_slot.begin(); iter != event_slot.end(); ++iter) {
        ...
        roo::log_info("process event %s, count %d", events_metric.c_str(), static_cast<int>(events_info.size()));

        std::map<std::string, stat_info_t> tag_info;
        calc_event_info_each_metric(events_info, tag_info);

        // roo::log_info("process reuslt for event: %s", events_name.c_str());
        for (auto it = tag_info.begin(); it != tag_info.end(); ++it) {
            roo::log_info("tag %s, count %d, value %ld",
                          it->first.c_str(),
