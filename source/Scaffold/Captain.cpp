...
bool Captain::init(const std::string& cfgFile) {
    ...
    std::shared_ptr<Service> monitor_task_service = std::make_shared<MonitorTaskService>("MonitorTaskService");
    if (!monitor_task_service || !monitor_task_service->init()) {
        log_err("create XtraTaskService failed.");
        return false;
    }
    Dispatcher::instance().register_service(ServiceID::MONITOR_TASK_SERVICE, monitor_task_service);
    ...
    if (!EventRepos::instance().init()) {
        log_err("Init event repos failed.");
        return false;
    }
